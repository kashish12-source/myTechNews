import { useState, useEffect } from 'react';
import { Newspaper, Layers, TableProperties } from 'lucide-react';
import NewsFeed from './components/NewsFeed';
import LlmComponents from './components/LlmComponents';
import ReplacementMatrix from './components/ReplacementMatrix';

export default function App() {
  const [activeTab, setActiveTab] = useState<'feed' | 'llm' | 'matrix'>('feed');
  const [serverOnline, setServerOnline] = useState<boolean | null>(null);

  // Check backend server status on load
  useEffect(() => {
    const checkServer = async () => {
      try {
        const res = await fetch('http://localhost:3001/api/news');
        if (res.ok) {
          setServerOnline(true);
        } else {
          setServerOnline(false);
        }
      } catch (e) {
        setServerOnline(false);
      }
    };
    checkServer();
    const interval = setInterval(checkServer, 10000);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="dashboard-grid">
      {/* Narrow, minimalist Icon-Only Sidebar */}
      <aside className="glass" style={{ 
        padding: '1.5rem 0', 
        borderRight: '1px solid var(--border-color)', 
        borderLeft: 'none', 
        borderTop: 'none', 
        borderBottom: 'none', 
        borderRadius: '0',
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'space-between',
        alignItems: 'center',
        height: '100vh',
        position: 'sticky',
        top: 0,
        zIndex: 50,
        width: '80px'
      }}>
        {/* Top Logo (Iconised) */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div className="tooltip tooltip-right" style={{ cursor: 'default' }}>
            <span style={{ 
              width: '36px', 
              height: '36px', 
              borderRadius: '6px', 
              background: 'linear-gradient(135deg, var(--accent-blue), var(--accent-cyan))', 
              color: '#090d16',
              display: 'flex', 
              alignItems: 'center', 
              justifyContent: 'center', 
              fontWeight: 700, 
              fontSize: '1rem',
              fontFamily: "'Space Grotesk', sans-serif"
            }}>
              TN
            </span>
            <span className="tooltip-text">TechNews Intelligence v1.0</span>
          </div>
        </div>

        {/* Center Navigation Icons */}
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', alignItems: 'center', width: '100%' }}>
          <div className="tooltip tooltip-right">
            <button 
              className="btn btn-icon"
              onClick={() => setActiveTab('feed')}
              style={{
                background: activeTab === 'feed' ? 'var(--accent-blue)' : 'transparent',
                color: activeTab === 'feed' ? '#ffffff' : 'var(--text-secondary)',
                border: activeTab === 'feed' ? 'none' : '1px solid var(--border-color)'
              }}
            >
              <Newspaper size={18} />
            </button>
            <span className="tooltip-text">Daily Updates Feed</span>
          </div>

          <div className="tooltip tooltip-right">
            <button 
              className="btn btn-icon"
              onClick={() => setActiveTab('llm')}
              style={{
                background: activeTab === 'llm' ? 'var(--accent-blue)' : 'transparent',
                color: activeTab === 'llm' ? '#ffffff' : 'var(--text-secondary)',
                border: activeTab === 'llm' ? 'none' : '1px solid var(--border-color)'
              }}
            >
              <Layers size={18} />
            </button>
            <span className="tooltip-text">LLM Core Components</span>
          </div>

          <div className="tooltip tooltip-right">
            <button 
              className="btn btn-icon"
              onClick={() => setActiveTab('matrix')}
              style={{
                background: activeTab === 'matrix' ? 'var(--accent-blue)' : 'transparent',
                color: activeTab === 'matrix' ? '#ffffff' : 'var(--text-secondary)',
                border: activeTab === 'matrix' ? 'none' : '1px solid var(--border-color)'
              }}
            >
              <TableProperties size={18} />
            </button>
            <span className="tooltip-text">AI Tool Replacement Matrix</span>
          </div>
        </nav>

        {/* Bottom Status & Bookmarks */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', alignItems: 'center', width: '100%' }}>
          {/* Quick Bookmarks */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', alignItems: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem', width: '60%' }}>
            <div className="tooltip tooltip-right">
              <a 
                href="https://news.ycombinator.com/" 
                target="_blank" 
                rel="noreferrer" 
                style={{ 
                  color: 'var(--text-secondary)', 
                  display: 'flex', 
                  alignItems: 'center', 
                  justifyContent: 'center',
                  width: '32px',
                  height: '32px',
                  borderRadius: '50%',
                  background: 'rgba(255, 102, 0, 0.08)',
                  border: '1px solid rgba(255, 102, 0, 0.15)',
                  fontSize: '0.75rem',
                  fontWeight: 'bold',
                  textDecoration: 'none'
                }}
              >
                Y
              </a>
              <span className="tooltip-text">Open Hacker News</span>
            </div>

            <div className="tooltip tooltip-right">
              <a 
                href="https://lobste.rs/" 
                target="_blank" 
                rel="noreferrer" 
                style={{ 
                  color: 'var(--text-secondary)', 
                  display: 'flex', 
                  alignItems: 'center', 
                  justifyContent: 'center',
                  width: '32px',
                  height: '32px',
                  borderRadius: '50%',
                  background: 'rgba(144, 0, 0, 0.08)',
                  border: '1px solid rgba(144, 0, 0, 0.15)',
                  fontSize: '0.75rem',
                  fontWeight: 'bold',
                  textDecoration: 'none'
                }}
              >
                L
              </a>
              <span className="tooltip-text">Open Lobsters</span>
            </div>

            <div className="tooltip tooltip-right">
              <a 
                href="https://www.paulgraham.com/articles.html" 
                target="_blank" 
                rel="noreferrer" 
                style={{ 
                  color: 'var(--text-secondary)', 
                  display: 'flex', 
                  alignItems: 'center', 
                  justifyContent: 'center',
                  width: '32px',
                  height: '32px',
                  borderRadius: '50%',
                  background: 'rgba(255, 255, 255, 0.04)',
                  border: '1px solid var(--border-color)',
                  fontSize: '0.75rem',
                  fontWeight: 'bold',
                  textDecoration: 'none'
                }}
              >
                PG
              </a>
              <span className="tooltip-text">Open Paul Graham Essays</span>
            </div>
          </div>

          {/* Connection Status Light */}
          <div className="tooltip tooltip-right" style={{ cursor: 'default' }}>
            <div style={{ 
              width: '10px', 
              height: '10px', 
              borderRadius: '50%', 
              background: serverOnline ? 'var(--accent-emerald)' : 'var(--accent-rose)',
              boxShadow: serverOnline ? '0 0 8px var(--accent-emerald)' : '0 0 8px var(--accent-rose)'
            }}></div>
            <span className="tooltip-text">
              {serverOnline ? 'Connected to local aggregation engine (ONLINE)' : 'Server Offline. Using pre-seeded news cache.'}
            </span>
          </div>
        </div>
      </aside>

      {/* Main Workspace Frame */}
      <main className="container" style={{ overflowY: 'auto', height: '100vh' }}>
        {activeTab === 'feed' && <NewsFeed />}
        {activeTab === 'llm' && <LlmComponents />}
        {activeTab === 'matrix' && <ReplacementMatrix />}
      </main>
    </div>
  );
}
