          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[250px]">
            ...
            {selectedCategory === 'all' && !searchQuery && heroArticle && (
              <div className="col-span-1 md:col-span-2 row-span-2 bg-slate-900/40 border border-slate-850 hover:border-red-500/35 rounded-2xl overflow-hidden transition-all duration-300 flex flex-col justify-between group relative shadow-2xl hover:shadow-[0_20px_50px_rgba(239,68,68,0.06)] hover:scale-[1.002]">
                ...
                <div className="h-48 w-full bg-slate-950 relative flex items-center justify-center border-b border-slate-850/60 overflow-hidden select-none">
                  ...
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_120%,rgba(239,68,68,0.15),transparent_50%),radial-gradient(circle_at_20%_-20%,rgba(6,182,212,0.12),transparent_40%)]"></div>
                  
                  <div className="z-10 text-center flex flex-col items-center px-4">
                    <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[9px] font-mono tracking-widest uppercase bg-red-500/10 border border-red-500/20 text-red-500 mb-3 animate-pulse">
                      <Sparkles size={8} />
                      <span>Featured Story</span>
                    </div>
                    <h3 className="text-white font-serif text-xl md:text-2xl font-bold max-w-lg px-4 leading-tight tracking-wide line-clamp-2 drop-shadow-md">
                      {heroArticle.title}
                    </h3>
                  </div>
                </div>
                ...
                <div className="p-6 flex-1 flex flex-col justify-between bg-slate-900/30">
                  ...
                  <div className="flex justify-between items-center mt-5 pt-4 border-t border-slate-850/60">
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="flex items-center gap-1.5 text-xs text-slate-450 hover:text-white border border-slate-800 hover:border-slate-700 bg-slate-950/80 px-3 py-1.5 rounded-lg transition-all"
                    >
                      <span>Source</span>
            ...
            {selectedCategory === 'all' && !searchQuery && (
              <div className="col-span-1 row-span-2 bg-slate-900/40 border border-slate-850 rounded-2xl p-5 flex flex-col gap-4 shadow-xl">
                ...
                <div className="flex flex-col divide-y divide-slate-850/50 overflow-y-auto flex-grow pr-1 scrollbar-none">
                  {trendingArticles.map((art, idx) => (
                    <div key={art.id} className="py-3.5 first:pt-0 last:pb-0 relative pl-8 group">
                      <span className="absolute left-0 top-3 text-2xl font-black font-mono text-slate-800/40 group-hover:text-red-500/25 transition-colors select-none">
                        0{idx + 1}
                      </span>
                      
                      <div className="flex items-center gap-2 text-[9px] text-slate-500 mb-1 select-none">
                        <span className="font-bold uppercase truncate max-w-[80px] text-slate-400">{art.source}</span>
                        <span>•</span>
                        <span className="font-mono">{formatDate(art.date)}</span>
                      </div>
            ...
            {selectedCategory === 'all' && !searchQuery && (
              <div className="col-span-1 row-span-1 bg-slate-900/40 border border-slate-850 hover:border-cyan-500/30 rounded-2xl p-5 flex flex-col justify-between shadow-lg relative group transition-all duration-300">
                <div className="absolute inset-0 bg-gradient-to-tr from-cyan-600/5 via-transparent to-transparent pointer-events-none"></div>
                <div>
                  <div className="flex justify-between items-center text-slate-555 mb-2 select-none">
                    <span className="text-[9px] uppercase font-bold tracking-wider font-mono text-slate-500">system.status</span>
                  ...
                  <div className="text-3xl font-black text-white font-mono mt-1 flex items-baseline gap-1">
                    {articles.length}
                    <span className="text-xs font-semibold text-slate-550 font-sans">articles</span>
                  </div>
                  
                  {/* Visual Scraping Logs/Bars Grid */}
                  <div className="flex gap-1 mt-3.5 h-3.5 items-end">
                  ...
                  <p className="text-[9px] text-slate-500 mt-2.5 font-mono flex justify-between select-none">
                    <span>Uptime: 99.98%</span>
                    <span>Latency: 124ms</span>
                  </p>
                </div>

                <div className="flex justify-between items-center border-t border-slate-850/60 pt-3 mt-2">
                  <span className="text-[9px] font-mono text-slate-500 truncate max-w-[120px]">
                    Updated: {lastUpdated ? formatDate(lastUpdated) : 'Never'}
                  </span>
                  <button 
                    onClick={handleRefresh}
                    className="p-1 hover:bg-slate-950 hover:text-cyan-400 text-slate-400 border border-slate-850 hover:border-slate-800 rounded transition-all select-none cursor-pointer"
            ...
            {selectedCategory === 'all' && !searchQuery && (
              <div className="col-span-1 row-span-1 bg-slate-900/40 border border-slate-850 hover:border-red-500/30 rounded-2xl p-5 flex flex-col justify-between shadow-lg relative group transition-all duration-300 select-none cursor-pointer">
                <div className="absolute inset-0 bg-gradient-to-bl from-red-600/5 via-transparent to-transparent pointer-events-none"></div>
                <div>
                  <div className="flex justify-between items-center text-slate-550 mb-2">
                    <span className="text-[9px] uppercase font-bold tracking-wider font-mono text-slate-500">ai.matrix</span>
                    <ShieldCheck size={14} className="text-slate-605 group-hover:text-red-500 transition-colors" />
                  </div>
                  <h4 className="text-sm font-serif font-extrabold text-white leading-snug">
                  ...
                  <div className="flex flex-col gap-1.5 mt-3">
                    <div className="flex items-center justify-between text-[8px] text-slate-500 font-mono">
                      <span>UX Design</span>
                      <span className="text-slate-400">15x</span>
                    </div>
                    <div className="w-full bg-slate-950 h-1 rounded-full overflow-hidden">
                      <div className="bg-red-500 h-full rounded-full transition-all duration-500 w-[95%] group-hover:bg-red-400"></div>
                    </div>
                    
                    <div className="flex items-center justify-between text-[8px] text-slate-500 font-mono">
                      <span>Boilerplate APIs</span>
                      <span className="text-slate-400">8x</span>
                    </div>
                    <div className="w-full bg-slate-950 h-1 rounded-full overflow-hidden">
                      <div className="bg-red-500 h-full rounded-full transition-all duration-500 w-[75%] group-hover:bg-red-400"></div>
                    </div>
                  </div>
                </div>

                <div className="border-t border-slate-850/60 pt-3 flex items-center justify-between text-xs text-red-500 font-bold group-hover:text-red-400 transition-colors mt-2">
